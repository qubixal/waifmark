...
> Note: Waifmark's exact test bank is **proprietary** and is not released for the public. 
...
Then add `--mode pipeline` to start/stop the backend (vLLM, llama.cpp) which serves the model.

### Scoring Judge

To speed up the scoring process, you would need to set an LLM judge.

By default, this is done through Openrouter:
...
> The agentic testing is conducted through a **harness**, not a **sandbox**. Tasks are run locally on your machine, and the model-under-test can only invoke allowed shell commands. This includes `python3` by default, so set `agentic.allow_python3_tool: false` if you don't trust it.